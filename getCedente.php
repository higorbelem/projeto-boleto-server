<?php
   ...
   $senha = $_POST['senha'];
	...
	$sql1 = $dbcon -> query("SELECT ce.`id` as 'id-cedente', ce.`nome` as 'nome-cedente', ce.`uso-banco`, ce.`use-santander`, ce.`uf`, ce.`cidade`, ce.`bairro`, ce.`rua`, ce.`numero`, ce.`cep`, ce.`cnpj`, ce.`informacoes`, ce.`contato`, ce.`valor-por-metro-cubico`, ce.`valor-minimo`, ce.`esgoto`, ce.`email`, ce.`logo`, co.`id` as 'id-conta', co.`banco`, co.`cip`, co.`conta`, co.`convenio`, co.`modalidade`, co.`agencia`, co.`codigo-empresa` FROM `cedentes` ce INNER JOIN `contas` co ON co.`cedente-id` = ce.`id` AND ce.`cnpj` = '$cnpj' AND ce.`senha` = '$senha'");
